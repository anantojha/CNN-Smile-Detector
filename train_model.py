...
#   Description:    Script to generate trained model from SMILEs dataset
# ------------------------------------------------------------------------------------------
import argparse
import cv2
import os
...
ap.add_argument('-m', '--model', required=True, help='output model name')
...
# account for skew in dataset
classTotals = labels.sum(axis=0)
classWeight = dict()
for i in range(0, len(classTotals)):
    classWeight[i] = classTotals.max() / classTotals[i]

# partition the dataset (80% Training, 20% Testing/Validation)
